...
import OnlyTrackerSelector from '../MovementTable/OnlyTrackerSelector';
...
export default function LostList(props) {
  ...
  const makeList = useCallback(() => {
    ...
    const outer = list.filter(ele => {
      return ele.mapName == map.name
    })
    console.log(outer);


    const newArr = outer.map(ele =>{
        const arr = ele.time.split(" ")
        const newobj = {name : ele.mapName, start : arr[0], end : arr[2]}
        console.log(newobj)
        return newobj
      })
  
      setLocationList(outer)
